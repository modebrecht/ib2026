...
# A4 scenes: delegate key down/up/reset to the exact shared A1 mechanics.
p=Path('tk2/a4Scenes.js')
s=p.read_text(encoding='utf-8')
old="function keyDown(key){var base=key.getAttribute('data-base')||'';transition(key,'transform 150ms ease,filter 150ms ease');transform(key,base+' translate(0 4)');key.style.filter='drop-shadow(0 0 8px rgba(96,165,250,.8))';}"
new="function keyDown(key){window.tk2SceneKeycaps.down(key,transition,'utility');}"
assert old in s, 'A4 keyDown anchor missing'
s=s.replace(old,new,1)
old="function keyUp(key){var base=key.getAttribute('data-base')||'';transition(key,'transform 140ms ease,filter 140ms ease');transform(key,base);key.style.filter='';}"
new="function keyUp(key){window.tk2SceneKeycaps.up(key);}"
assert old in s, 'A4 keyUp anchor missing'
s=s.replace(old,new,1)
# A4 contains a second scene implementation with same helpers; replace all remaining matching helpers.
s=s.replace("function keyDown(key){var base=key.getAttribute('data-base')||'';transition(key,'transform 150ms ease,filter 150ms ease');transform(key,base+' translate(0 4)');key.style.filter='drop-shadow(0 0 8px rgba(96,165,250,.8))';}","function keyDown(key){window.tk2SceneKeycaps.down(key,transition,'utility');}")
s=s.replace("function keyUp(key){var base=key.getAttribute('data-base')||'';transition(key,'transform 140ms ease,filter 140ms ease');transform(key,base);key.style.filter='';}","function keyUp(key){window.tk2SceneKeycaps.up(key);}")
p.write_text(s,encoding='utf-8')
