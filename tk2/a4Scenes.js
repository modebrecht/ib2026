(function(){
  ...
  function createWindowResizeScene(container,type,options){
    ...
    container.innerHTML=''
      +'<svg data-window-resize-scene="'+type+'" viewBox="0 0 680 320" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Animation: '+(isMax?'Fenster maximieren':'Fenster verkleinern oder minimieren')+'">'
      ...
      +'<g class="key-row" transform="translate(449 225)"><g class="resize-key" data-base=""><rect width="70" height="43" rx="10" fill="#172033" stroke="#475569" stroke-width="1.5"/><text x="35" y="27" text-anchor="middle" font-family="Arial" font-size="14" font-weight="800" fill="#dbeafe">Win</text></g><text x="83" y="27" font-family="Arial" font-size="16" font-weight="900" fill="#64748b">+</text><g class="resize-key" data-base="translate(102 0)" transform="translate(102 0)"><rect width="55" height="43" rx="10" fill="#172033" stroke="#475569" stroke-width="1.5"/><text x="27.5" y="28" text-anchor="middle" font-family="Arial" font-size="20" font-weight="800" fill="#dbeafe">'+(isMax?'↑':'↓')+'</text></g></g>'
    ...
    var svg=container.querySelector('svg'),win=svg.querySelector('.window'),direction=svg.querySelector('.direction'),toast=svg.querySelector('.toast'),keys=Array.from(svg.querySelectorAll('.resize-key'));
  ...
  function createA4Scene(container,options){
    ...
    var mode=options.mode||'newTab',cfg=CONFIG[mode]||CONFIG.newTab,active=options.autoplay!==false,autoLoop=options.loop!==false,reduceMotion=window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches,uid='tk2a4'+(++counter),timers=[],running=false,controller=null;
