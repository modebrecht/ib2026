(function(){
  ...
  function renderSummary(){var s=loadProgress().A,host=byId('summaryRows');host.innerHTML='';var row=document.createElement('div'),q=document.createElement('div'),name=document.createElement('div'),vals=document.createElement('div');row.className='summary-row';q.className='summary-q';q.textContent='Q8';q.style.color=META.theme.accent;name.textContent=META.name;vals.className='summary-vals';if(!s)vals.textContent='noch offen';else{var parts=['1. '+s.first+' %'];if(typeof s.second==='number')parts.push('2. '+s.second+' %');parts.push('Best '+s.best+' %');vals.textContent=parts.join(' · ');}vals.style.color=s?scoreColor(s.best):'var(--text-muted)';row.appendChild(q);row.appendChild(name);row.appendChild(vals);host.appendChild(row);}
