(function(){
  ...
  var QUEST_LABELS={
    ...
    7:'Merkblatt gesichert',
    8:'Programme & Browser',9:'Windows & Arbeitsalltag',
    10:'Wiederholung A1',11:'Wiederholung A2',12:'Wiederholung A4 + A5',13:'Alles gemischt'
  };
  var SHEET_TITLES={A1:'Allgemeine Tastenkürzel',A2:'Sonderzeichen mit AltGr',A3:'Merkblatt zu Tastenkürzeln',A4:'Programme & Browser',A5:'Windows & Arbeitsalltag',A6:'Wiederholen & festigen'};
  ...
  function a3Rows(){
    var progress=parse(A3_PROGRESS_KEY),scores=getScores(),second=progress.second;
    var legacyDone=(typeof second==='string'&&second.trim().length>0)||(typeof second==='number'&&isFinite(second));
    var completed=progress.completed===true||legacyDone||(scores.q7||0)>=100;
    if(!completed)return[];
    return[{q:7,label:QUEST_LABELS[7],value:'Nachschlagewerk heruntergeladen und für spätere Aufgaben gesichert.'}];
  }
  ...
  function syncButton(){if(!pdfButton)return;var available=hasData(scope);pdfButton.disabled=!available;pdfButton.style.opacity=available?'1':'.52';pdfButton.style.cursor=available?'pointer':'not-allowed';pdfButton.title=available?'PDF mit aktuellem Zwischenstand erzeugen':'PDF verfügbar, sobald mindestens eine Aufgabe bearbeitet oder A3 gesichert wurde';if(scope==='all'){var sheets=selectedSheets('all'),quests=sheets.reduce(function(n,s){return n+s.rows.length;},0),state=document.getElementById('pdf-module-state'),meta=document.getElementById('pdf-module-meta');if(state)state.textContent=quests?quests+' Eintr'+(quests===1?'ag':'äge')+' erfasst':'noch keine Ergebnisse';if(meta)meta.textContent=quests?sheets.length+' Arbeitsblatt'+(sheets.length===1?'':'blätter')+' · '+quests+' Eintr'+(quests===1?'ag':'äge'):'Noch keine Ergebnisse';}}
