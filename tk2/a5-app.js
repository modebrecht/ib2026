(function(){
  ...
  var SETS=Object.keys(DATA),STORAGE_KEY='tk_a5_progress_v1',currentSet='A',attemptFinished=false,freshAttempt=false;
  ...
  function saveProgress(key,pct,answers,correct){var d=loadProgress(),o=d[key]||{};d[key]={first:typeof o.first==='number'?o.first:pct,last:pct,best:Math.max(typeof o.best==='number'?o.best:0,pct),answers:answers,lastCorrect:correct};localStorage.setItem(STORAGE_KEY,JSON.stringify(d));}
  ...
  function render(forceFresh){
    var m=META[currentSet];setTheme(m);renderTabs();sheetTitle.textContent=m.title;sheetDesc.textContent=m.desc;questionsContainer.innerHTML='';
    ...
    qs.forEach(function(q,i){
      ...
      card.className='question-card';title.className='question-title';label.className='question-label';label.textContent='Frage '+(i+1);title.appendChild(label);
      ...
      select.className='answer-select';select.dataset.correct=q.correct;empty.value='';empty.textContent='Bitte wählen …';select.appendChild(empty);shuffle([q.correct].concat(q.wrong)).forEach(function(v){var o=document.createElement('option');o.value=v;o.textContent=v;select.appendChild(o);});
    ...
    if(completed){var c=typeof stored.lastCorrect==='number'?stored.lastCorrect:Math.round(stored.last/100*qs.length);scoreBox.textContent='Letzter Versuch: '+c+' / '+qs.length+' richtig ('+stored.last+' %)';scoreBox.style.color=scoreColor(stored.last);checkBtn.textContent='↻ Neuer Versuch';attemptFinished=true;}
    else{scoreBox.textContent=(startFresh?'Neuer Versuch: ':'')+'0 / '+qs.length+' richtig';scoreBox.style.color='var(--text-muted)';checkBtn.textContent='✅ Überprüfen';attemptFinished=false;}
  ...
  downloadPdfBtn.addEventListener('click',downloadPdf);finishPdfBtn.addEventListener('click',downloadPdf);
  render();renderSummary();updateCompletion();
})();