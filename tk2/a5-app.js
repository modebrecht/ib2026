(function(){
  ...
  // scoreId keeps the previous q11-q14 storage keys so existing progress/XP survives.
  var META={
    A:{q:10,scoreId:11,name:'A1',title:'Quest 10 – Wiederholung A1',desc:'Allgemeine Tastenkürzel aus A1.',theme:BLUE},
    B:{q:11,scoreId:12,name:'A2',title:'Quest 11 – Wiederholung A2',desc:'Sonderzeichen mit AltGr aus A2. Das Zielzeichen bleibt immer sichtbar.',theme:AMBER},
    C:{q:12,scoreId:13,name:'A4',title:'Quest 12 – Wiederholung A4',desc:'Alle zwölf Browser-, Programm- und Windows-Kürzel aus A4.',theme:CYAN},
    D:{q:13,scoreId:14,name:'Gemischt',title:'Quest 13 – Alles gemischt',desc:'A1, A2 und A4 in gemischten Alltagssituationen.',theme:GREEN}
  };
  ...
  function evaluate(){
    ...
    var pct=Math.round(correct/ss.length*100);saveProgress(currentSet,pct,chosen,correct);var qid='q'+META[currentSet].scoreId;if(typeof saveQuestScore==='function')saveQuestScore(qid,pct);var xp=typeof awardQuestImprovementXP==='function'?awardQuestImprovementXP(qid,correct,5):0;scoreBox.textContent=correct+' / '+ss.length+' richtig ('+pct+' %)'+(xp?' · +'+xp+' XP':'');scoreBox.style.color=scoreColor(pct);attemptFinished=true;freshAttempt=false;checkBtn.textContent='↻ Neuer Versuch';renderSummary();updateCompletion();
  }
  ...
  function downloadPdf(){
    var data=loadProgress();if(!allAttempted()){alert('Bearbeite zuerst Q10 bis Q13 mindestens einmal.');return;}var student=typeof requireStudentName==='function'?requireStudentName():'';if(!student)return;var rows=SETS.map(function(k){return{q:META[k].q,name:META[k].name,first:data[k].first,best:data[k].best};});var avg=Math.round(rows.reduce(function(s,r){return s+r.best;},0)/rows.length),canvas=document.createElement('canvas');canvas.width=1200;canvas.height=850;var ctx=canvas.getContext('2d'),g=ctx.createLinearGradient(0,0,1200,850);g.addColorStop(0,'#0f172a');g.addColorStop(1,'#1e293b');ctx.fillStyle=g;ctx.fillRect(0,0,1200,850);ctx.strokeStyle='#10b981';ctx.lineWidth=6;ctx.strokeRect(30,30,1140,790);ctx.textAlign='center';ctx.fillStyle='#6ee7b7';ctx.font='800 20px "Space Grotesk", sans-serif';ctx.fillText('INFORMATIK B25 – ARBEITSBLATT A5',600,95);ctx.fillStyle='#fff';ctx.font='800 36px "Space Grotesk", sans-serif';ctx.fillText('WIEDERHOLUNG TASTENKÜRZEL',600,155);ctx.fillStyle='#94a3b8';ctx.font='500 18px sans-serif';ctx.fillText('Erster Versuch → bester Versuch · Quest 10–13',600,195);ctx.fillStyle='rgba(16,185,129,.15)';ctx.strokeStyle='#10b981';ctx.lineWidth=2;ctx.beginPath();ctx.roundRect(250,220,700,65,16);ctx.fill();ctx.stroke();ctx.fillStyle='#6ee7b7';ctx.font='800 28px "Space Grotesk", sans-serif';ctx.fillText(student,600,262);ctx.fillStyle='rgba(15,23,42,.85)';ctx.beginPath();ctx.roundRect(100,340,1000,350,20);ctx.fill();var y=405;ctx.textAlign='left';rows.forEach(function(r){ctx.fillStyle='#e2e8f0';ctx.font='700 20px "Space Grotesk", sans-serif';ctx.fillText('Q'+r.q+' ('+r.name+')',150,y);ctx.textAlign='right';ctx.fillStyle=r.best>=80?'#10b981':r.best>=50?'#f59e0b':'#ef4444';ctx.fillText(r.first+' % → '+r.best+' %',1050,y);ctx.textAlign='left';y+=60;});ctx.textAlign='center';ctx.fillStyle='#10b981';ctx.font='800 24px "Space Grotesk", sans-serif';ctx.fillText('Ø bester Versuch: '+avg+' %',600,735);ctx.fillStyle='#94a3b8';ctx.font='500 15px sans-serif';ctx.fillText('Wiederholung Tastenkürzel · Informatik B25',600,790);function make(){var pdf=new window.jspdf.jsPDF({orientation:'landscape',unit:'mm',format:[297,210]});pdf.addImage(canvas.toDataURL('image/png'),'PNG',0,0,297,210);var safe=typeof sanitizeStudentNameForFileName==='function'?sanitizeStudentNameForFileName(student):student.replace(/[^a-zA-Z0-9]/g,'_');pdf.save('A5_Wiederholung_'+safe+'.pdf');}if(window.jspdf)make();else{var s=document.createElement('script');s.src='../tk/vendor/jspdf.umd.min.js';s.onload=make;document.head.appendChild(s);}
  }
